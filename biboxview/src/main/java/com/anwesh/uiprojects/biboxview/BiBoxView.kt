...
class BiBoxView(ctx : Context) : View(ctx) {
    ...
    data class State(var scale : Float = 0f, var dir : Float = 0f, var prevScale : Float = 0f) {
        ...
        fun update(cb : (Float) -> Unit) {
            scale += scale.updateValue(dir, 1, boxes)
    ...
    data class BBNode(var i : Int, val state : State = State()) {
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawBBNode(i, state.scale, paint)
    ...
    data class Renderer(var view : BiBoxView) {

        private val animator : Animator = Animator(view)
        private val bibox : BiBox = BiBox(0)

        fun render(canvas : Canvas, paint : Paint) {
            canvas.drawColor(backColor)
            bibox.draw(canvas, paint)
            animator.animate {
                bibox.update {i, scl ->
                    animator.stop()
                }
